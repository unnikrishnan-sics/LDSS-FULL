        ...
        try {
            const token = localStorage.getItem('token');
             ...
             const quizResponse = await axios.get(
                 `http://localhost:4000/ldss/quizzes/${quizId}`, // Use the new route
             ...
             const attemptsResponse = await axios.get(
                 `http://localhost:4000/ldss/parent/quizzes/child/${fetchedQuiz.childId}`, // Reuse the list endpoint to check attempts
                 { headers: { Authorization: `Bearer ${token}` } }
             );
    ...
    const handleSubmitAttempt = async () => {
        ...
        try {
            ...
            const response = await axios.post(
                `http://localhost:4000/ldss/parent/quiz/submit/${quizId}`,
